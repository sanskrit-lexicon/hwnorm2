...
def spellmerge(recs_preverb1,mwrecs,pwmwd):
 ...
 for k1 in d1:
  r1 = d1[k1]
  k2 = None
  if k1 in pwmwd:  # prefer the mapping, if present
   k = pwmwd[k1].k1mw
   if k in d2:
    k2 = k
    pwmw = pwmwd[k1]
...
def unused_spellmerge(recs_preverb1,mwrecs,pwmwd):
 keys1,d1 = k1map(recs_preverb1)
 keys2,d2 = k1map(mwrecs)
 # get exact matches
 matches = []
...
if __name__=="__main__": 
 ...
 filein1 = sys.argv[3] # mwverbs
 filein2 = sys.argv[4] # pw_mw_map (spelling changes)
 
 fileout = sys.argv[5] # preverb2.txt  matches of filein, filein1
 ...
 mwrecs = init_mwverbs(filein1)
 pw_mw_recs,pwmwd = init_pw_mw_map(filein2)
 spellmerge(recs_preverb1,mwrecs,pwmwd)
